...
from Distributions.Slash_dist import Slash, Slash_left, Slash_right, Slash_between


def draw_slash(input_frame, fig, canvas):
    ...
    def plot():
        ...
        if selection == "Curve":
            ...
            prob = Slash_left(ax, param=float(spinboxes[0].get()))
            text_widget.delete("1.0", tk.END)
            text_widget.insert(tk.END, f"P(X ≤ {float(spinboxes[0].get())}) = {prob:.4f}")
        elif selection == "P(X≥x)":
            prob = Slash_right(ax, param=float(spinboxes[0].get()))
            text_widget.delete("1.0", tk.END)
            text_widget.insert(tk.END, f"P(X ≥ {float(spinboxes[0].get())}) = {prob:.4f}")
        elif selection == "P(a≤X≤b)":
            prob = Slash_between(ax, param1=float(spinboxes[0].get()), param2=float(spinboxes[1].get()))
            text_widget.delete("1.0", tk.END)
