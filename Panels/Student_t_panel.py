...
from Distributions.Student_t_dist import student_t, student_t_left, student_t_right, student_t_between


def draw_student_t(input_frame, fig, canvas):
    tk.Label(input_frame, text="Degrees of freedom (ν)").pack()
    entry_df = tk.Spinbox(input_frame, from_=1, to=1000, textvariable=tk.StringVar(value="5"))
    entry_df.pack()
    ...
    def plot():
        ...
        if selection == "Curve":
            ...
            prob_X = student_t_left(ax, df=df, param=float(spinboxes[0].get()))
            text_widget.delete("1.0", tk.END)
            text_widget.insert(tk.END, f"P(X ≤ {float(spinboxes[0].get())}) = {prob_X:.4f}")
        elif selection == "P(X≥x)":
            prob_X = student_t_right(ax, df=df, param=float(spinboxes[0].get()))
            text_widget.delete("1.0", tk.END)
            text_widget.insert(tk.END, f"P(X ≥ {float(spinboxes[0].get())}) = {prob_X:.4f}")
        elif selection == "P(a≤X≤b)":
            prob_X = student_t_between(ax, df=df, param1=float(spinboxes[0].get()), param2=float(spinboxes[1].get()))
            text_widget.delete("1.0", tk.END)
