...
from Distributions.Cauchy_dist import cau, cau_left, cau_right, cau_between


def draw_cauchy(input_frame, fig, canvas):
    tk.Label(input_frame, text="Location (x₀)").pack()
    entry_loc = tk.Spinbox(input_frame, from_=-1000, to=1000, textvariable=tk.StringVar(value="0"))
    entry_loc.pack()
    tk.Label(input_frame, text="Scale (γ)").pack()
    ...
    def plot():
        ...
        if selection == "Curve":
            cau(ax, loc=loc, scale=scale)
        elif selection == "P(X≤x)":
            prob_X = cau_left(ax, loc=loc, scale=scale, param=float(spinboxes[0].get()))
            text_widget.delete("1.0", tk.END)
            text_widget.insert(tk.END, f"P(X ≤ {float(spinboxes[0].get())}) = {prob_X:.4f}")
        elif selection == "P(X≥x)":
            prob_X = cau_right(ax, loc=loc, scale=scale, param=float(spinboxes[0].get()))
            text_widget.delete("1.0", tk.END)
            text_widget.insert(tk.END, f"P(X ≥ {float(spinboxes[0].get())}) = {prob_X:.4f}")
        elif selection == "P(a≤X≤b)":
            prob_X = cau_between(ax, loc=loc, scale=scale, param1=float(spinboxes[0].get()), param2=float(spinboxes[1].get()))
            text_widget.delete("1.0", tk.END)
